...
static void menu_create_cb(lv_obj_t *obj)
{
    ...
    if(!lang_txt_is_arabic())
        tileview_register_all_menu(obj, ui_act_id_null, ui_act_id_null, \
            prev_act_id, ui_act_id_null, ui_act_id_find_phone);
    else
        tileview_register_all_menu(obj, ui_act_id_null, ui_act_id_null, \
            ui_act_id_null, prev_act_id, ui_act_id_find_phone);
...
static void menu_display_cb(lv_obj_t *obj)
{
    ...
    widget_label_para.label_text = get_lang_txt_with_id(lang_txtid_search_phone);
    lv_obj_t *find_phone_label = common_widget_label_create(&widget_label_para);
    lv_obj_align(find_phone_label, LV_ALIGN_TOP_MID, 0, 132);
