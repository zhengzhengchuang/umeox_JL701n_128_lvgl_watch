...
int JL_rcsp_update_cmd_resp(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    case JL_OPCODE_GET_DEVICE_UPDATE_FILE_INFO_OFFSET:
        if (0 == len) {
            ...
            lcd_st77903_singal_to_continue();
    ...
    case JL_OPCODE_SET_DEVICE_REBOOT:
        ...
        if (support_dual_bank_update_en) {
            rcsp_update_before_jump_handle(0);
        }
        break;
    default:
        ret = 1;
...
static void JL_resp_inquire_device_if_can_update(u8 OpCode, u8 OpCode_SN, u8 update_sta)
{
    ...
    // 关闭混响
    if (0 == update_sta) {
        extern void smartbox_close_reverbrateion_state_and_update(void);
        smartbox_close_reverbrateion_state_and_update();
    }
}
...
int JL_rcsp_update_msg_deal(void *hdl, u8 event, u8 *msg)
{
    ...
    case MSG_JL_INQUIRE_DEVEICE_IF_CAN_UPDATE:
        ...
        if (0 == can_update_flag && 0 == support_dual_bank_update_en) {
    ...
    case MSG_JL_LOADER_DOWNLOAD_START:
        ...
        if (support_user_file_update_v2_en) {
            extern void user_file_flash_update_once_req_size_set(u32 pack_size);
            user_file_flash_update_once_req_size_set(update_once_req_size);
        } else
#endif
        {
        ...
        if (support_dual_bank_update_en) {
            ...
            if (!get_update_ex_flash_flag()) {
#if USER_FILE_UPDATE_V2_EN
                if (support_user_file_update_v2_en) {
                    register_user_chip_update_v2_handle(NULL);
                } else
#endif
                {
                ...
                if (smartbox_eflash_update_flag_get()) {
                    extern void app_smartbox_task_disable_opt(void);
                    app_smartbox_task_disable_opt();
