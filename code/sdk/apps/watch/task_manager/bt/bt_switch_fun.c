...
int a2dp_media_packet_user_handler(u8 *data, u16 size)
{
    ...
    if (1) { // 不允许后台切回。例如在闹钟/传输等任务的时候会被打断
        local_irq_enable();
        return -EINVAL;
    }
...
static u8 bt_background_exit()
{
    ...
    if (bt_media_is_running()) {
        __this->last_play_stu = 1;
    } else {
        __this->last_play_stu = 0;
    }
    a2dp_dec_close();
...
#endif /* #if TCFG_USER_EMITTER_ENABLE */
    __this->cmd_flag = 0;
    if (__this->last_play_stu) {
        u8 a2dp_stu = a2dp_get_status();
        y_printf("a2dp_stu :%x \n", a2dp_stu);
        if (a2dp_stu == BT_MUSIC_STATUS_STARTING) {
            user_send_cmd_prepare(USER_CTRL_AVCTP_OPID_PLAY, 0, NULL);
            os_time_dly(2);
            user_send_cmd_prepare(USER_CTRL_AVCTP_OPID_PLAY, 0, NULL);
        } else {
            u8 call_stu = get_call_status();
            if ((call_stu == BT_CALL_OUTGOING) || (call_stu == BT_CALL_ALERT)
                || (call_stu == BT_CALL_INCOMING) || (call_stu == BT_CALL_ACTIVE)
               ) {
            } else {
                user_send_cmd_prepare(USER_CTRL_AVCTP_OPID_PLAY, 0, NULL);
            }
...
static int bt_close_bredr_do(int priv)
{
    ...
    if (__this->bt_close_bredr == 1) {
        extern void bt_close_bredr_timeout_stop();
...
static void bt_timeout_close_bredr(void *priv)
{
    ...
    /* if (get_remote_test_flag() && (0 == get_rcsp_connect_status())) { */
    /*     return; */
    /* } */
#endif
    if (do_cnt != bt_bredr_cnt) {
    ...
    if (BT_STATUS_WAITINT_CONN != get_emitter_connect_status()) {
        printf("bt connect:0x%x \n", get_emitter_connect_status());
        user_send_cmd_prepare(USER_CTRL_WRITE_SCAN_DISABLE, 0, NULL);
        user_send_cmd_prepare(USER_CTRL_WRITE_CONN_DISABLE, 0, NULL);
        return ;
    }
...
static int bredr_conn_dev_do(int priv)
{
    ...
    user_send_cmd_prepare(USER_CTRL_START_CONNEC_VIA_ADDR, 6, ptr->mac);
    free(ptr);
    return 0;
}
