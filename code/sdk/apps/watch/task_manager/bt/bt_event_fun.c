...
static u32 bt_phone_active_start_time = 0;
...
{
    if (!bt_user_priv_var.auto_connection_counter && \
        get_current_poweron_memory_search_index(bt_user_priv_var.auto_connection_addr)) {
...
#if (TCFG_BD_NUM == 2)
    if((get_total_connect_dev() == 1) && bt->value) 
    {   //当前有一台连接上了,并且连接的是sink设备
        user_send_cmd_prepare(USER_CTRL_WRITE_SCAN_ENABLE, 0, NULL);
        user_send_cmd_prepare(USER_CTRL_WRITE_CONN_ENABLE, 0, NULL);
    }
...
extern void audio_aec_reboot(u8 reduce);
#endif

/*----------------------------------------------------------------------------*/
/**@brief       蓝牙通话事件kws相关处理
   @param       event - 通话事件（INCOME, ACTIVE, HANGUP）
   @return      无
   @note
*/
/*----------------------------------------------------------------------------*/
...
{
    return bt_phone_active_start_time;
}
...
/*----------------------------------------------------------------------------*/
/**@brief  蓝牙status 手机来电电话本名
   @param
   @return
   @note
*/
/*----------------------------------------------------------------------------*/
u8 phone_name_text[30];
void bt_status_phone_name(struct bt_event *bt)
{
    u8 *phone_name = NULL;
    u8 phone_name_len = 0;
    phone_name = (u8 *)bt->value;
    phone_name_len = strlen((const char *)phone_name);
    if (phone_name_len > 30) {   //限制显示30个字节,可根据需求增加
        phone_name_len = 30;
    }
    /* put_buf(phone_name, phone_name_len); */
    memset(phone_name_text, '\0', phone_name_len + 1);
...
/* ***************************************************************************/
/**
 * @brief   ：display_call_dly
 延时显示通话界面，用于通话中切换通话链路的界面变化
 */
/* ***************************************************************************/
void display_call_dly()
{
    if (__this->call_back_flag & BIT(1)) {
    ...
    if (bt->event == HCI_EVENT_VENDOR_REMOTE_TEST) {
        if (0 == bt->value) {
            ...
            if (ble_update_get_ready_jump_flag()) {
                log_info("not reset in ready jump\n");
                return 0;
            }
#endif
