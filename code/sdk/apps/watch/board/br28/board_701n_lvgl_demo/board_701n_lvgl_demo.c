...
NORFLASH_SFC_DEV_PLATFORM_DATA_BEGIN(norflash_usr_dev_data)
    .sfc_spi_pdata = &sfc_spi_data,
    .start_addr = 0,//FLASH_SIZE_4M+FLASH_SIZE_2M,//CONFIG_EXTERN_FLASH_SIZE/2,
    .size = CONFIG_EXTERN_FLASH_SIZE - CONFIG_EXTERN_USER_VM_FLASH_SIZE,//FLASH_SIZE_8M+FLASH_SIZE_2M,//CONFIG_EXTERN_FLASH_SIZE - CONFIG_EXTERN_FLASH_SIZE/2,
NORFLASH_SFC_DEV_PLATFORM_DATA_END()
...
REGISTER_DEVICES(device_table) = {
    ...
    // { "fat_nor",   &norflash_sfc_dev_ops , (void *)&norflash_norfs_fat_dev_data},
     { "fat_nor",   &norflash_sfc_fs_dev_ops , (void *)&norflash_norfs_fat_dev_data},
#endif
    { "usr_nor",   &norflash_sfc_fs_dev_ops , (void *)&norflash_usr_dev_data},
...
u8 get_power_on_status(void)
{
    ...
    if (adkey_data.enable) {
        u32 adc_value = \
            adc_get_value(adkey_data.ad_channel);
        //printf("adc_value = %d\n", adc_value);
        if(adc_value > 150 && adc_value < 350) {
            return 1;
        }
    }
#endif
...
//add user
    ...
    if (lcd_spi_data.pin_bl	!= NO_CONFIG_PORT) {
        gpio_direction_output(lcd_spi_data.pin_bl, 0);
		soff_gpio_protect(lcd_spi_data.pin_bl);
	}
