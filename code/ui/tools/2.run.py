...
# 定义输入文件夹和输出文件夹的路径
input_folder = "ex_dat_tmp"
output_folder = "ex_bin_tmp"
...
# clear_file_contents(f"./ex_bin_out/{head_name}_addresses.txt")
clear_file_contents(f"./ex_bin_out/{head_name}_index.c")
clear_file_contents(f"./ex_bin_out/{head_name}.bin")
...
struct_text += f"const struct ex_file_index_t {head_name}_index[] = " + "{\n"

define_text = f'#ifndef __EX_FILE_INDEX_H__ \n'
define_text += f'#define __EX_FILE_INDEX_H__ \n'
define_text += '''
#ifdef __cplusplus
extern "C" {
#endif
...
with open(f"./ex_bin_out/{head_name}.bin", "wb") as o_file:
    o_file.write(output_bin)

struct_text += "};"

with open(f"./ex_bin_out/{head_name}_index.c", "w") as struct_file:
...
define_text += f'#endif'
# print(define_text)
with open(f"./ex_bin_out/{head_name}_index.h", "w") as define_file:
